...
const NavigationTabs = () => {
  ...
  const handleResponseByType = (responseType, reply, fullReplyItem, checkConfirmation) => {
    switch (responseType) {
      case "simple":
        setMessages((prev) => [
          ...prev,
          ...
          reply = reply.map((item) => {
            if (typeof item === "object" && item !== null) {
              return Object.fromEntries(
                Object.entries(item).map(([key, val]) => [key.replace(/_([a-z])/g, (g) => g[1].toUpperCase()), val]),
              )
            }
            return item
          })
  ...
  useEffect(() => {
    ...
    newSocket.on("connection_status", (data) => {
      if (data.status !== "connected") {
        setMessages((prev) => [{ wallet: "System", content: data.status, isHistory: false }, ...prev])
      }
    })

    newSocket.on("error", (data) => {
      setMessages((prev) => [...prev, { wallet: "Error", content: data.message, isHistory: false }])
    })
    ...
    newSocket.on("chat_response", (rawData) => {
      ...
      if (data?.response_type === "chatoshi") {
        handleResponseByType("chatoshi", data.data.replies, data, checkConfirmation)
        return
      }
      ...
      const transactionReplies = replies.filter((item) => item.response_type === "transaction")

      const nonTransactionReplies = replies.filter((item) => item.response_type !== "transaction")

      nonTransactionReplies.forEach((replyItem, index) => {
        const { reply, response_type } = replyItem
        handleResponseByType(response_type, reply, replyItem, checkConfirmation)
      })

      if (transactionReplies.length > 0) {
        if (transactionReplies.length === 1 && !hasHandledIntents.current.intent0) {
          setPendingAction({
  ...
  const handleConfirmation = (confirmed) => {
    ...
    const updatedProcessedIntents = [...processedIntents, { index: pendingAction.intentIndex, confirmed }]
    ...
    setMessages((prev) => [
      ...
      {
        wallet: "Chat",
        content: `Transaction ${pendingAction.intentIndex + 1} ${confirmed ? "confirmed" : "denied"}`,
        isJson: false,
    ...
    const transactionReplies =
      allIntentsData?.data?.replies?.filter((item) => item.response_type === "transaction") || []
  ...
  const sendMessage = (text = message) => {
    if (text.trim() && socket) {
      ...
      setMessages((prev) => [...prev, { wallet: "You", content: text, isHistory: false }])
      setMessage("")
      setLoading(true)
  ...
  const GetHistoryChat = async () => {
    try {
      ...
      const chatRes = await FireApi(`/get-chat-sessions/${userId}/${userInfo.sessionId}`, "GET", null, chatHistoryUrl)
  ...
  const processApiResponse = (apiResponse) => {
    ...
    apiResponse.data.messages.forEach((msg) => {
      try {
        ...
        let responseData
        try {
          responseData = typeof msg.response === "string" ? JSON.parse(msg.response) : msg.response
        } catch (parseError) {
        ...
        if (responseData?.data?.replies) {
          responseData.data.replies.forEach((replyItem) => {
            ...
            switch (responseType) {
              case "simple":
                content = typeof replyContent === "string" ? replyContent.trim() : JSON.stringify(replyContent, null, 2)
                break
              ...
              case "all_portfolios":
                ...
                additionalProps = {
                  responseType: "all_portfolios",
                  portfolioResponse: replyContent.all_portfolios || replyContent,
              ...
              case "all_transactions":
                ...
                additionalProps = {
                  responseType: "all_transactions",
                  transactionResponse: replyContent.all_transactions || replyContent,
                  portfolioTitle: replyContent.title || replyContent,
                  isJson: false,
                }
                break
  ...
  const getActionIcon = () => {
    if (message.trim().length > 0) {
      ...
      return recording ? (
        ...
        <img src="/recording-01.png" alt="record" className={`${isMobile ? "w-4 h-4" : "w-5 h-5"}`} />
      )
    }
  }
 ...
 return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col min-h-screen w-full overflow-auto">
        ...
        <div className={`flex-1 overflow-hidden ${isMobile ? 'pb-24' : ''}`}> {/* Mobile par hi padding */}
          <div
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    ) : msg.wallet === "Chat" && msg.responseType === "get_top_cryptos" ? (
                      <CryptoDisplay data={msg.cryptoData} title={msg.cryptoTitle} />
                    ) : msg.wallet === "Chat" && msg.responseType === "get_user_balance" ? (
                      <UserBalance data={msg.chainData} />
                    ) : msg.wallet === "Chat" && msg.responseType === "get_token_balance" ? (
                        ...
                        {typeof msg.content === "object" && msg.content !== null ? (
                          <div>
                            <h4 className="font-bold mb-2">
                              {msg.content.action ? `Action: ${convertKeysToCamelCase(msg.content.action)}` : null}
                            </h4>
                            <div className="pl-4">
                              {Object.entries(msg.content)
                                .filter(([key, value]) => {
                                  if (value === "" || value === null || value === undefined) return false
                                  if (["success", "message", "status"].includes(key)) return false
                                ...
                                .map(([key, value]) => {
                                  ...
                                  let displayValue = typeof value === "object" && value !== null 
                                    ? JSON.stringify(convertKeysToCamelCase(value), null, 2)
              ...
              <div className="flex justify-start">
                <div className="px-3 py-2 rounded-xl text-xs md:text-sm text-gray-800 dark:text-gray-200">
                  <span className="inline-flex gap-1">
                    ...
                    <span className="w-2 h-2 rounded-full bg-gray-500 animate-bounce" style={{ animationDelay: "600ms" }}></span>
                  </span>
                </div>
              </div>
            )}
